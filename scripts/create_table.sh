...
do
    ...
    col_count=$(zenity --entry --title="Column Number" --text="Enter number of columns: ")
...
do
    ...
    do
        ...
        name=$(zenity --entry --title="Column Name" --text="Enter the name of column $((i+1)): ")
        # empty?
        if [ -z "$name" ]; then
    ...
    # column type
    col_types[i]=$(zenity --list --title="Attribute Datatype" --column="Data Types" "String" "Int")
    if [ $? -eq 0 ]; then
...
# choose primary key
# -------------------------
pk_choice=$(zenity --list --title="Choose Primary Key" --column="Attributes" "${col_names[@]}")
if [ $? -eq 0 ]; then
